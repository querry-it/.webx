...
function Xy() {
  ...
  const scrollXy = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const sc = scrollXy.current;
    if (!sc) return;

    const handleScroll = () => {
      const current = sc.scrollTop;
      if (!state.navbar_x.headerXy) {
        setState('SET_NAVBAR_X', 'headerXy', current > 0);
      } else {
        setState('SET_NAVBAR_X', 'headerXy', current > 60);
      }
    };
  ...
  return (
    <div ref={scrollXy} className={cx('location__prev')}>
