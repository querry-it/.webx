...
//                                     type="text"
//                                     ref={userRef}
//                                     className="mx-[44px] w-full h-full outline-none border-none
//                                                 outline-none
...
//                                 >
//                                     {active ? (
//                                         <i className="fas fa-eye-slash absolute top-1/2 -translate-y-2 -translate-x-7 text-[14px] mr-[5px] text-gray-500"></i>
//                                     ) : (
//                                         <i className="fas fa-eye absolute top-1/2 -translate-y-2 -translate-x-7 text-[14px] mr-[5px] text-gray-500"></i>
//                                     )}
//                                 </button>
//                             </div>
//                         </div>
...
export default function Login() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (token) {
                    const [loginRes] = await Promise.all([
                        axios.post("http://localhost:5000/auth/refresh", {}, { withCredentials: true }),
                        axios.post("http://localhost:5000/auth/clear-login", {}, { withCredentials: true }),
                    ]);
                    ...
                    await axios.post("http://localhost:5000/auth/clear-login", {}, { withCredentials: true });
                }
            } catch (error: any) {
                console.warn("Client refresh failed");
                if (!error.response) {
    ...
    return (
        ...
        <FormLayout title="Đăng nhập" onSubmit={handleSubmit} error={error}>
            ...
            <SwitchLink text="Bạn đã có tài khoản?." actionText="Tạo tài khoản" onClick={handleRegister} />
        </FormLayout>
    );
}